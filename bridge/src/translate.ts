...
export function responsesToChat(req: ResponsesRequest): {
  ...
  for (const item of items) {
    if (item.type === "message") {
      ...
      if (role === "assistant") {
        ...
        // user / system pass through with original role label.
        // `developer` role is collapsed into `system` because some
        // third-party Chat Completions proxies (e.g. AgentRouter) reject
        // it with `messages[N].role is invalid` — they only support the
        // legacy {system,user,assistant,tool} set.
        // Defensive: any other unknown role is coerced to "user" so the
        // upstream never sees a chat message without a role.
        let safeRole: "user" | "system";
        ...
        messages.push({ role: safeRole, content: content as string | unknown[] });
      }
  ...
  if (req.reasoning !== undefined && req.reasoning !== null) chat.reasoning = req.reasoning;
